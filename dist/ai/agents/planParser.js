...
exports.runPlanParserAgent = runPlanParserAgent;
...
function scoreSection(section) {
    ...
    if (/\b(electrical|elec|sheet\s+e\d|panel\s+schedule|legend)\b/.test(lower))
        score += 6;
    if (/\b(outlet|receptacle|switch|fixture|lighting|detector|fan)\b/.test(lower))
        score += 4;
    if (/\b(general notes|building standards|structural notes)\b/.test(lower))
        score -= 2;
    return score;
}
function buildFocusedPlanText(rawText, maxChars = MAX_TEXT_PROMPT_CHARS) {
    ...
    for (const index of ordered) {
        ...
        if ((combined + nextChunk).length > maxChars)
            break;
        combined += nextChunk;
    }
    if (!combined) {
...
function buildTextPrompt(text, learningHints) {
...
function buildImagePrompt(learningHints) {
    return [
        {
            type: 'text',
...
function extractCountFromNotes(notes, label) {
    if (!notes)
        return null;
...
function reconcileCountsFromNotes(extraction) {
    ...
    extraction.detectedItems.outlets = bumpMax(extraction.detectedItems.outlets, extractCountFromNotes(notes, 'outlets') ?? extractCountFromNotes(notes, 'receptacles'));
    extraction.detectedItems.switches = bumpMax(extraction.detectedItems.switches, extractCountFromNotes(notes, 'switches'));
    extraction.detectedItems.lightFixtures = bumpMax(extraction.detectedItems.lightFixtures, extractCountFromNotes(notes, 'light fixtures'));
    extraction.detectedItems.recessedLights = bumpMax(extraction.detectedItems.recessedLights, extractCountFromNotes(notes, 'recessed lights'));
    extraction.detectedItems.ceilingFans = bumpMax(extraction.detectedItems.ceilingFans, extractCountFromNotes(notes, 'ceiling fans'));
    extraction.detectedItems.smokeDetectors = bumpMax(extraction.detectedItems.smokeDetectors, extractCountFromNotes(notes, 'smoke detectors'));
    extraction.detectedItems.carbonMonoxideDetectors = bumpMax(extraction.detectedItems.carbonMonoxideDetectors, extractCountFromNotes(notes, 'carbon monoxide detectors'));
    extraction.detectedItems.exhaustFans = bumpMax(extraction.detectedItems.exhaustFans, extractCountFromNotes(notes, 'exhaust fans'));
    extraction.legendItems.vacancySensors = bumpMax(extraction.legendItems.vacancySensors, extractCountFromNotes(notes, 'vacancy sensor') ?? extractCountFromNotes(notes, 'vacancy sensors'));
    extraction.legendItems.fluorescentFixtures = bumpMax(extraction.legendItems.fluorescentFixtures, extractCountFromNotes(notes, 'fluorescent') ?? extractCountFromNotes(notes, 'fluorescent fixtures'));
    extraction.legendItems.wallMountedFixtures = bumpMax(extraction.legendItems.wallMountedFixtures, extractCountFromNotes(notes, 'wall mounted') ?? extractCountFromNotes(notes, 'wall-mounted fixtures'));
    extraction.legendItems.indoorAirVentFans = bumpMax(extraction.legendItems.indoorAirVentFans, extractCountFromNotes(notes, 'air vent') ?? extractCountFromNotes(notes, 'indoor air ventilation fan'));
    extraction.legendItems.gfciOutlets = bumpMax(extraction.legendItems.gfciOutlets, extractCountFromNotes(notes, 'gfci') ?? extractCountFromNotes(notes, 'gfi'));
    extraction.detectedItems.switches = bumpMax(extraction.detectedItems.switches, extraction.legendItems.vacancySensors);
    extraction.detectedItems.lightFixtures = bumpMax(extraction.detectedItems.lightFixtures, extraction.legendItems.fluorescentFixtures);
    extraction.detectedItems.lightFixtures = bumpMax(extraction.detectedItems.lightFixtures, extraction.legendItems.wallMountedFixtures);
    extraction.detectedItems.exhaustFans = bumpMax(extraction.detectedItems.exhaustFans, extraction.legendItems.indoorAirVentFans);
    extraction.detectedItems.outlets = bumpMax(extraction.detectedItems.outlets, extraction.legendItems.gfciOutlets);
}
// ─── Gemini helper — handles large PDFs (and images) natively ─────────────────
async function runWithGemini(data, mimeType, learningHints) {
...
async function runPlanParserAgent(input, iterationCount) {
    if (iterationCount >= schemas_1.MAX_AGENT_ITERATIONS) {
        throw new Error(`PlanParserAgent exceeded max iterations (${schemas_1.MAX_AGENT_ITERATIONS})`);
    }
    let response;
    const learningHints = (0, learningProfile_1.getLearningPromptHints)();
                    ...
                    const completion = await openai.chat.completions.create({
                        ...
                        messages: [
                            { role: 'system', content: SYSTEM_PROMPT },
                            {
                                role: 'user',
                                content: [
                                    ...buildImagePrompt(learningHints),
    ...
    if (!cleaned.startsWith('{')) {
        ...
        return schemas_1.PlanExtractionSchema.parse({
            squareFootage: null,
    ...
    const extraction = schemas_1.PlanExtractionSchema.parse(parsed);
    extraction.legendItems = extraction.legendItems ?? {
        vacancySensors: null,
        fluorescentFixtures: null,
        wallMountedFixtures: null,
