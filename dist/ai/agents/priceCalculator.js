...
exports.runPriceCalculatorAgent = runPriceCalculatorAgent;
...
function isSmallScope(scope) {
    const deviceCount = scope.outlets +
        scope.switches +
        scope.lightFixtures +
...
function estimateTargetSubtotal(scope) {
    const smokeCo = scope.smokeCoDetectors;
    const panel = (scope.mainPanelUpgrade ? 4200 : 0) +
        (scope.subPanelAmps ? 2000 : 0);
    const wiringCircuitCount = Math.max(4, Math.ceil((scope.outlets + scope.switches + scope.lightFixtures + scope.recessedLights + scope.ceilingFans) / 5) +
        scope.dedicatedCircuits);
    const wiring = wiringCircuitCount * 300;
    const devices = scope.outlets * 95 +
        scope.switches * 85 +
...
function calibrateEstimate(scope, parsed) {
    ...
    const maxWiringQty = Math.max(4, Math.ceil((scope.outlets + scope.switches + scope.lightFixtures + scope.recessedLights + scope.ceilingFans) / 4) +
        scope.dedicatedCircuits);
    ...
    let subtotal = parsed.lineItems.reduce((s, i) => s + (Number(i.total) || 0), 0);
    ...
    if (subtotal > rangeHigh * 1.05 || subtotal < rangeLow * 0.75) {
        ...
        subtotal = parsed.lineItems.reduce((s, i) => s + (Number(i.total) || 0), 0);
    }
    ...
    if (parsed.total < rangeLow)
        parsed.competitivePosition = 'below';
    else if (parsed.total > rangeHigh)
        parsed.competitivePosition = 'above';
    else
        parsed.competitivePosition = 'competitive';
...
async function runPriceCalculatorAgent(scope, iterationCount) {
    if (iterationCount >= schemas_1.MAX_AGENT_ITERATIONS) {
        throw new Error(`PriceCalculatorAgent exceeded max iterations (${schemas_1.MAX_AGENT_ITERATIONS})`);
    }
...
- All IDs must be unique strings`;
    ...
    if (calibrated.lineItems) {
        calibrated.lineItems = calibrated.lineItems.map((item) => ({
            ...item,
            id: item.id && item.id.length > 5 ? item.id : (0, uuid_1.v4)(),
        }));
    }
    return schemas_1.PricedEstimateSchema.parse(calibrated);
}
