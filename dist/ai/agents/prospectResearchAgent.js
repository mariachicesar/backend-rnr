...
exports.runProspectResearchAgent = runProspectResearchAgent;
...
async function fetchWebsiteText(url) {
    ...
    const timer = setTimeout(() => controller.abort(), b2bSchemas_1.WEBSITE_FETCH_TIMEOUT_MS);
    try {
        ...
        const text = stripHtml(html).slice(0, b2bSchemas_1.WEBSITE_CONTENT_MAX_CHARS);
        return text || null;
    }
    catch {
        return null;
...
async function runProspectResearchAgent(input, iterationCount) {
    if (iterationCount >= b2bSchemas_1.MAX_B2B_AGENT_ITERATIONS) {
        throw new Error(`ProspectResearchAgent exceeded max iterations (${b2bSchemas_1.MAX_B2B_AGENT_ITERATIONS})`);
    }
...
Return JSON:
{
  ...
  "recommendedAssetType": "gc_checklist" | "pm_compliance" | "dev_benchmark"
    ...
    return b2bSchemas_1.ResearchOutputSchema.parse(parsed);
}
//# sourceMappingURL=prospectResearchAgent.js.map