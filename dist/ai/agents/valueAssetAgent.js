...
exports.runValueAssetAgent = runValueAssetAgent;
...
async function runValueAssetAgent(input, iterationCount) {
    if (iterationCount >= b2bSchemas_1.MAX_B2B_AGENT_ITERATIONS) {
        throw new Error(`ValueAssetAgent exceeded max iterations (${b2bSchemas_1.MAX_B2B_AGENT_ITERATIONS})`);
    }
...
Return JSON:
{
  ...
  "callToAction": string (max 120 chars — one plain sentence, no ask, just availability)
    ...
    const validated = b2bSchemas_1.ValueAssetOutputSchema.parse(parsed);
 ...
 */
function sanitizeAssetHtml(html) {
    return html
...
//# sourceMappingURL=valueAssetAgent.js.map