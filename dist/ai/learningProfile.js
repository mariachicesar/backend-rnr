...
exports.recordScopeCorrection = recordScopeCorrection;
exports.getLearningPromptHints = getLearningPromptHints;
exports.getLearningProfileSummary = getLearningProfileSummary;
const fs_1 = __importDefault(require("fs"));
const path_1 = __importDefault(require("path"));
const PROFILE_DIR = path_1.default.join(process.cwd(), 'data');
const PROFILE_PATH = path_1.default.join(PROFILE_DIR, 'ai-learning-profile.json');
...
function loadProfile() {
    try {
        ...
        const raw = fs_1.default.readFileSync(PROFILE_PATH, 'utf8');
...
function saveProfile(profile) {
    ensureProfileDir();
    fs_1.default.writeFileSync(PROFILE_PATH, JSON.stringify(profile, null, 2), 'utf8');
}
function updateStat(stat, predicted, actual) {
...
function recordScopeCorrection(rawExtraction, confirmedScope) {
    if (!rawExtraction)
        return;
    ...
    for (const [key, p, a] of pairs) {
        ...
        if (next)
            profile.numeric[key] = next;
    }
    profile.totalSamples += 1;
    profile.updatedAt = new Date().toISOString();
    saveProfile(profile);
}
function getLearningPromptHints() {
    const profile = loadProfile();
