...
exports.orchestrateParsePlan = orchestrateParsePlan;
exports.orchestrateAnalyzeScope = orchestrateAnalyzeScope;
exports.orchestratePriceEstimate = orchestratePriceEstimate;
exports.orchestrateWriteDraft = orchestrateWriteDraft;
exports.orchestrateSaveEstimate = orchestrateSaveEstimate;
const database_1 = __importDefault(require("../config/database"));
const planParser_1 = require("./agents/planParser");
const scopeAnalyzer_1 = require("./agents/scopeAnalyzer");
const priceCalculator_1 = require("./agents/priceCalculator");
const estimateWriter_1 = require("./agents/estimateWriter");
const learningProfile_1 = require("./learningProfile");
const schemas_1 = require("./schemas");
// ─── Session helpers ──────────────────────────────────────────────────────────
async function loadSession(sessionId, userId) {
    const row = await database_1.default.aiEstimateSession.findFirst({
...
async function saveSession(state) {
    await database_1.default.aiEstimateSession.update({
...
async function orchestrateParsePlan(sessionId, userId, input) {
    const state = await loadSession(sessionId, userId);
    if (state.iterations.parser >= schemas_1.MAX_AGENT_ITERATIONS) {
    ...
    try {
        const extraction = await (0, planParser_1.runPlanParserAgent)(input, state.iterations.parser - 1);
...
async function orchestrateAnalyzeScope(sessionId, userId, confirmedScope, learningExtraction) {
    const state = await loadSession(sessionId, userId);
    ...
    if (confirmedScope) {
        ...
        (0, learningProfile_1.recordScopeCorrection)(learningExtraction ?? state.rawExtraction, confirmedScope);
    ...
    if (!state.rawExtraction)
        throw new Error('No plan extraction found. Run plan parser first.');
    if (state.iterations.scope >= schemas_1.MAX_AGENT_ITERATIONS) {
    ...
    try {
        const scope = await (0, scopeAnalyzer_1.runScopeAnalyzerAgent)(state.rawExtraction, state.iterations.scope - 1);
        state.scope = scope;
        state.step = 'price';
        state.error = null;
        await saveSession(state);
...
async function orchestratePriceEstimate(sessionId, userId) {
    const state = await loadSession(sessionId, userId);
    if (!state.scope)
        throw new Error('No confirmed scope found. Run scope analyzer first.');
    if (state.iterations.price >= schemas_1.MAX_AGENT_ITERATIONS) {
    ...
    try {
        const priced = await (0, priceCalculator_1.runPriceCalculatorAgent)(state.scope, state.iterations.price - 1);
...
async function orchestrateWriteDraft(sessionId, userId, confirmedLineItems) {
    const state = await loadSession(sessionId, userId);
    if (!state.scope)
        throw new Error('No scope found.');
    if (!state.lineItems)
        throw new Error('No priced line items found.');
    ...
    if (state.iterations.writer >= schemas_1.MAX_AGENT_ITERATIONS) {
    ...
    try {
        const draft = await (0, estimateWriter_1.runEstimateWriterAgent)(state.scope, state.lineItems, state.iterations.writer - 1);
        state.draft = draft;
        state.step = 'draft';
        state.error = null;
        await saveSession(state);
...
async function orchestrateSaveEstimate(sessionId, userId, clientId, validUntilDays = 30) {
    const state = await loadSession(sessionId, userId);
    if (!state.draft)
        throw new Error('No draft found. Run estimate writer first.');
    // Generate estimate number
    const latest = await database_1.default.estimate.findFirst({ orderBy: { createdAt: 'desc' } });
    ...
    const estimate = await database_1.default.estimate.create({
        data: {
            ...
            notes: state.draft.notes + (state.draft.exclusions?.length
                ? '\n\nEXCLUSIONS:\n' + state.draft.exclusions.map((e) => `• ${e}`).join('\n')
                : ''),
            status: 'draft',
        },
    });
