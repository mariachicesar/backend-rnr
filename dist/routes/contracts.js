...
async function generateContractNumber() {
    const latestContract = await database_1.default.contract.findFirst({
...
function describeMilestone(contract, milestone, phases) {
...
async function generateMilestoneInvoicesForContract(contractId, userId) {
    ...
    const existingMilestoneIds = new Set(contract.invoices
        .map((invoice) => {
    ...
    for (const [index, milestone] of schedule.entries()) {
        ...
        const notes = (0, billing_1.mergeBillingMarkers)('Auto-generated from contract payment schedule.', {
...
router.post('/', async (req, res) => {
    const { estimateId, clientId, title, description, items, phases, paymentSchedule, subtotal, tax, total, startDate, completionDate, terms, } = req.body;
    if (!clientId || !title) {
    ...
    try {
        ...
        const refreshed = await database_1.default.contract.findUnique({
...
router.post('/:id/generate-invoices', async (req, res) => {
    try {
...
router.put('/:id', async (req, res) => {
    const { title, description, items, phases, paymentSchedule, subtotal, tax, total, startDate, completionDate, status, terms, signedByClient, } = req.body;
    try {
        const contract = await database_1.default.contract.findUnique({ where: { id: req.params.id } });
        if (!contract || contract.userId !== req.user.id) {
            return res.status(404).json({ error: 'Contract not found' });
        }
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        await database_1.default.contract.delete({ where: { id: req.params.id } });
