...
exports.isGoogleCalendarSyncEnabled = isGoogleCalendarSyncEnabled;
exports.createGoogleCalendarEventForAppointment = createGoogleCalendarEventForAppointment;
...
function getCalendarAuth() {
    ...
    if (process.env.GOOGLE_USE_ADC === 'true') {
        return new googleapis_1.google.auth.GoogleAuth({ scopes: CALENDAR_SCOPE });
    }
    ...
    if (oauthClientId && oauthClientSecret && oauthRefreshToken) {
        const oauth2Client = new googleapis_1.google.auth.OAuth2(oauthClientId, oauthClientSecret);
...
function appointmentSummary(type) {
...
async function createGoogleCalendarEventForAppointment(payload) {
    const calendarId = process.env.GOOGLE_CALENDAR_ID;
    const auth = getCalendarAuth();
    if (!calendarId || !auth) {
        return {
            status: 'skipped',
            error: 'Google Calendar not configured. Set GOOGLE_CALENDAR_ID plus one auth mode: (1) GOOGLE_SERVICE_ACCOUNT_EMAIL + GOOGLE_PRIVATE_KEY, (2) GOOGLE_OAUTH_CLIENT_ID + GOOGLE_OAUTH_CLIENT_SECRET + GOOGLE_OAUTH_REFRESH_TOKEN, or (3) GOOGLE_USE_ADC=true after gcloud auth application-default login.',
        };
    }
