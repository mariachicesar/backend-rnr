...
export async function runLinkedInDraftAgent(
  input: LinkedInDraftInput,
  iterationCount: number
): Promise<LinkedInDraftOutput> {
  if (iterationCount >= MAX_B2B_AGENT_ITERATIONS) {
    throw new Error(`LinkedInDraftAgent exceeded max iterations (${MAX_B2B_AGENT_ITERATIONS})`);
  }
...
Return JSON:
{
  ...
  return LinkedInDraftOutputSchema.parse(parsed);
}
