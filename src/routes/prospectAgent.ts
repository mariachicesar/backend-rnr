...
import {
  ...
  orchestrateResearch,
  orchestrateLinkedIn,
  orchestrateValueAsset,
  orchestrateOutreach,
} from '../ai/prospectOrchestrator';

const router = Router();

// ─── Discovery ────────────────────────────────────────────────────────────────
router.post('/discovery', async (req: AuthRequest, res: Response) => {
  try {
    const result = await orchestrateDiscovery(req.body);
    res.json(result);
  } catch (err: any) {
    if (err.name === 'ZodError') return res.status(400).json({ error: 'Invalid config', details: err.flatten?.() });
    if (err.statusCode === 409) return res.status(409).json({ error: err.message });
    res.status(500).json({ error: err.message ?? 'Discovery failed' });
  }
});

router.post('/discovery/cancel', async (req: AuthRequest, res: Response) => {
  try {
    const { searchType } = req.body ?? {};
    const result = await cancelDiscoveryRun(searchType);
    res.json(result);
  } catch (err: any) {
...
router.get('/discovery/runs', async (_req: AuthRequest, res: Response) => {
  const runs = await prisma.prospectAgentRun.findMany({
...
router.get('/discovery/trade-options', (_req: AuthRequest, res: Response) => {
  res.json({ items: CSLB_TRADE_OPTIONS });
});

// ─── Per-prospect agents ──────────────────────────────────────────────────────
router.post('/:prospectId/enrich', async (req: AuthRequest, res: Response) => {
  try {
    const output = await orchestrateEnrich(req.params.prospectId);
    res.json(output);
  } catch (err: any) {
...
router.post('/:prospectId/research', async (req: AuthRequest, res: Response) => {
  try {
    const output = await orchestrateResearch(req.params.prospectId);
    res.json(output);
  } catch (err: any) {
...
router.post('/:prospectId/linkedin', async (req: AuthRequest, res: Response) => {
  try {
    const output = await orchestrateLinkedIn(req.params.prospectId);
    res.json(output);
  } catch (err: any) {
...
router.post('/:prospectId/asset', async (req: AuthRequest, res: Response) => {
  try {
    const output = await orchestrateValueAsset(req.params.prospectId);
    res.json(output);
  } catch (err: any) {
...
router.post('/:prospectId/outreach', async (req: AuthRequest, res: Response) => {
  try {
    const output = await orchestrateOutreach(req.params.prospectId);
    res.json(output);
  } catch (err: any) {
